...
logging.basicConfig(filename=os.path.join(log_dir, "ekyc_logs.log"), level=logging.INFO, format=logging_str,
                    filemode="a")
    ...
    if image_file is not None:
        ...
        if face_image is not None:
            ...
            if is_face_verified:
                ...
                records = fetch_records( text_info)  # Ensure this function uses the conn parameter
                if records.shape[0] > 0:
                    st.write(records.shape)
                    st.write(records)
                is_duplicate = check_duplicacy(text_info)  # Ensure this function uses the conn parameter
                if is_duplicate:
                    st.write(f"User already present with ID {text_info['ID']}")
                else:
                    st.write(text_info)
                    text_info['DOB'] = text_info['DOB'].strptime('%Y-%m-%d')
                    text_info['Embedding'] = get_face_embeddings(face_image_path1)
                    insert_records( text_info)  # Ensure this function uses the conn parameter
...
def main():
    ...
    if image_file is not None:
        ...
        main_content(image_file, face_image_file, mydb)
